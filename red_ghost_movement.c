...
/*   Updated: 2023/07/28 19:50:53 by aajaanan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_red_ghost_continue(t_params *params)
{
    ...
    if (params->red.direction == 0 && new_row > 0 && params->map.data[new_row - 1][new_col] != '1')
		new_row--;
    else if (params->red.direction == 1 && new_col < params->map.cols - 1 && params->map.data[new_row][new_col + 1] != '1')
        new_col++;
    else if (params->red.direction == 2 && new_row < params->map.rows - 1 && params->map.data[new_row + 1][new_col] != '1')
        new_row++;
