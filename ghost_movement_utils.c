...
/*   Updated: 2023/07/29 11:31:57 by aajaanan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	has_multiple_options(t_params *params, int row, int col)
{
	...
	while (i < 4)
	{
		new_col = col;
		new_row = row;
		if (directions[i] == 0 && new_row > 0 && params->map.data[new_row - 1]
			[new_col] != '1')
			new_row--;
		else if (directions[i] == 1 && new_col < params->map.cols - 1
			&& params->map.data[new_row][new_col + 1] != '1')
			new_col++;
		else if (directions[i] == 2 && new_row < params->map.rows - 1
			&& params->map.data[new_row + 1][new_col] != '1')
			new_row++;
		else if (directions[i] == 3 && new_col > 0
			&& params->map.data[new_row][new_col - 1] != '1')
			new_col--;
