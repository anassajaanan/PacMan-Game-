...
void	move_blue_ghost_continue(t_params *params)
{
	...
	new_col = params->blue.col;
	new_row = params->blue.row;
	if (params->blue.direction == 0 && new_row > 0 && params->map.data
		[new_row - 1][new_col] != '1')
		new_row--;
	else if (params->blue.direction == 1 && new_col < params->map.cols - 1
		&& params->map.data[new_row][new_col + 1] != '1')
		new_col++;
	else if (params->blue.direction == 2 && new_row < params->map.rows - 1
		&& params->map.data[new_row + 1][new_col] != '1')
		new_row++;
	else if (params->blue.direction == 3 && new_col > 0
		&& params->map.data[new_row][new_col - 1] != '1')
		new_col--;
	params->blue.row = new_row;
	params->blue.col = new_col;
...
void	move_blue_ghost(t_params *params)
{
	...
	{
		counter = 0;
		has_multiple = has_multiple_options(params, params->blue.row,
				params->blue.col);
		if (has_multiple)
